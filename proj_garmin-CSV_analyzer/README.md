...
![Version](https://img.shields.io/badge/Version-2.12-green.svg)
...
### 1. 深度汇总报表 (Macro View)
- **多级奖牌纪录墙 (Medal Wall)**：除 PB 外，新增银牌与铜牌纪录墙，自动计算并展示各距离段前三名成绩。
...
### 3. 三重纪录对比 (PB vs PW vs Medal)
- 提取各距离段（5k/10k/半马/全马）的金/银/铜奖牌记录及最慢记录。采用区间锁定逻辑，防止数据重叠干扰。
